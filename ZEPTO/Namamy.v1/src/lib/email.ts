...
export async function sendEmail({ to, subject, html, text }: EmailOptions) {
  try {
    ...
    if (!process.env.SMTP_USER || !process.env.SMTP_PASS || 
        process.env.SMTP_USER === 'contact@namamy.com' || 
        process.env.SMTP_PASS === 'your-app-password') {
      console.log('Email not configured - would send:', { to, subject });
      // Return success but don't actually send email in development
      return { success: true, messageId: 'dev-mode-' + Date.now(), skipped: true };
    }

    const info = await transporter.sendMail({
      from: `"Namamy" <${process.env.SMTP_USER}>`,
      to,
      subject,
      text,
      html
    ...
    return { success: true, error: 'Email sending failed but operation continued', skipped: true };
  }
}
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
      <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="text-align: center; margin: 30px 0;">
          <a href="${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:8080'}/shop" 
