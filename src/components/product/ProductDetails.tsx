...
const getImageAlt = (image: string | any, fallback: string): string => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-4">
          ...
          <div className="relative aspect-square bg-gray-100 rounded-lg overflow-hidden">
            <Image
              src={product.images && product.images.length > 0 ? getImageUrl(product.images[selectedImage]) || getImageUrl(product.images[0]) : '/images/placeholder-product.jpg'}
              alt={product.images && product.images.length > 0 ? getImageAlt(product.images[selectedImage], product.name) || getImageAlt(product.images[0], product.name) : product.name}
              fill
          ...
          {product.images && product.images.length > 1 && (
            <div className="flex space-x-2 overflow-x-auto pb-2">
              {product.images.map((image, index) => (
                <button
                  key={typeof image === 'string' ? `img-${index}` : image.id || `img-${index}`}
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center border border-gray-300 rounded-md">
                  ...
                  <Button
                    ...
                    disabled={quantity >= (product.stockQuantity || product.stockCount || 0)}
                  >
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{product.nutrition?.calories || product.nutritionInfo?.calories || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{t('nutrition.protein')}</span>
                    <span className="font-medium">{product.nutrition?.protein || product.nutritionInfo?.protein || 'N/A'}g</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{t('nutrition.totalFat')}</span>
                    <span className="font-medium">{product.nutrition?.totalFat || product.nutritionInfo?.totalFat || 'N/A'}g</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{t('nutrition.carbs')}</span>
                    <span className="font-medium">{product.nutrition?.totalCarbohydrates || product.nutritionInfo?.totalCarbohydrates || 'N/A'}g</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{product.nutrition?.calcium || product.nutritionInfo?.calcium || 'N/A'}mg</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{t('nutrition.iron')}</span>
                    <span className="font-medium">{product.nutrition?.iron || product.nutritionInfo?.iron || 'N/A'}mg</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{t('nutrition.potassium')}</span>
                    <span className="font-medium">{product.nutrition?.potassium || product.nutritionInfo?.potassium || 'N/A'}mg</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{t('nutrition.sodium')}</span>
                    <span className="font-medium">{product.nutrition?.sodium || product.nutritionInfo?.sodium || 'N/A'}mg</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{product.nutrition?.dietaryFiber || product.nutritionInfo?.dietaryFiber || 'N/A'}g</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{t('nutrition.sugars')}</span>
                    <span className="font-medium">{product.nutrition?.totalSugars || product.nutritionInfo?.totalSugars || 'N/A'}g</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{t('nutrition.cholesterol')}</span>
                    <span className="font-medium">{product.nutrition?.cholesterol || product.nutritionInfo?.cholesterol || 'N/A'}mg</span>
                  </div>
                </CardContent>
              </Card>
            </div>
